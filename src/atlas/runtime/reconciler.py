...
class PositionReconciler:
    """Restore and verify open positions from journal + exchange balance."""
    ...
    def reconcile_on_startup(self) -> tuple[Position | None, dict[str, Any]]:
        ...
        if journal_pos and broker_pos:
            if broker_pos.quantity <= DUST_QTY:
                meta.update(
                    {
                        "source": "journal_mismatch",
                        "warning": "journal_open_but_exchange_flat",
                    }
                )
                return None, meta
        ...
        if broker_pos and not journal_pos:
            ...
            meta.update(
                {
                    "source": "exchange_orphan",
                    "warning": "exchange_balance_without_journal_entry",
                }
            )
    ...
    def reconcile_periodic(self, current: Position | None) -> tuple[Position | None, dict[str, Any]]:
        ...
        if has_engine and not has_broker:
            ...
            meta["warning"] = "engine_long_but_exchange_flat"
            return None, meta
        ...
        if not has_engine and has_broker:
            ...
            meta["warning"] = "engine_flat_but_exchange_long"
            return broker_pos, meta
        ...
        if has_engine and has_broker and abs(current.quantity - broker_pos.quantity) > DUST_QTY:
            ...
            meta["qty"] = {"engine": current.quantity, "broker": broker_pos.quantity}
            return updated, meta
