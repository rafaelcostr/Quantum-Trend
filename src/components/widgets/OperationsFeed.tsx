...
function eventColor(event: string, action?: string | null) {
  if (event === "entry" || action === "entry") return "text-success border-success/30 bg-success/10";
  if (event === "exit" || action === "exit") return "text-destructive border-destructive/30 bg-destructive/10";
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3 text-xs">
        <span className={`inline-flex items-center gap-1.5 chip ${running ? (mode === "live" ? "text-destructive" : "text-success") : ""}`}>
          <Radio className={`h-3 w-3 ${running ? "animate-pulse" : ""}`} />
          {running ? (mode === "live" ? "LIVE" : "PAPER") : "PARADO"}
        </span>
        {running && (
          <>
            <span className="text-muted-foreground">Ticks: <span className="num text-foreground">{bot.ticks ?? 0}</span></span>
            <span className="text-muted-foreground">Último: <span className="num">{fmtTime(bot.last_tick_at)}</span></span>
            {poll_seconds != null && next_tick_in != null && (
              <span className="text-muted-foreground">Próximo tick ~<span className="num">{next_tick_in}s</span></span>
            )}
      ...
      <ul className={`space-y-2 ${compact ? "max-h-72" : "max-h-[28rem]"} overflow-y-auto pr-1`}>
        <AnimatePresence initial={false}>
          {items.length === 0 && running ? (
            <li className="text-sm text-muted-foreground py-4 text-center">Aguardando primeiro tick…</li>
          ) : (
            items.map((item, i) => (
              <motion.li
                ...
                className={`rounded-xl border px-3 py-2.5 text-sm ${eventColor(item.event, item.action)}`}
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <span className="text-[10px] uppercase tracking-wider opacity-70">{item.event}</span>
                  ...
                  <div className="text-right shrink-0 text-[10px] opacity-70">
                    ...
                    {item.equity != null && <div className="num mt-0.5">${Number(item.equity).toLocaleString(undefined, { maximumFractionDigits: 0 })}</div>}
                  </div>
                </div>
              </motion.li>
            ))
