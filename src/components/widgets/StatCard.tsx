...
export function StatCard({
  label, value, delta, icon: Icon, data, accent = "primary",
}: {
  label: string;
  value: string;
  delta?: number;
  ...
  return (
    ...
    >
      <div className="absolute -top-12 -right-12 h-32 w-32 rounded-full blur-3xl opacity-30" style={{ background: color }} />
      ...
      <div className="h-12 mt-3 -mx-2">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={series}>
            ...
            <Area type="monotone" dataKey="y" stroke={color} strokeWidth={2} fill={`url(#${gradId})`} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </motion.div>
