...
import { Bar, BarChart, ResponsiveContainer, XAxis, YAxis, CartesianGrid, Tooltip, Cell, Line, LineChart } from "recharts";
...
function Page() {
  ...
  if (error || !data) return <div className="text-destructive text-sm">Erro ao carregar relatórios.</div>;
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data.monthly_returns}>
                ...
                <Bar dataKey="r" radius={[6, 6, 0, 0]}>
                  {data.monthly_returns.map((d, i) => <Cell key={i} fill={d.r >= 0 ? "#7C3AED" : "#EF4444"} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={data.equity_curve}>
                ...
                <YAxis tick={{ fill: "#64748b", fontSize: 11 }} axisLine={false} tickLine={false} />
                <Tooltip contentStyle={{ background: "rgba(12,16,28,0.95)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12 }} />
                <Line type="monotone" dataKey="equity" stroke="#3B82F6" strokeWidth={2.5} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </Panel>
