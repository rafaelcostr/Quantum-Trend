    ...
    <div className="space-y-8">
        ...
        actions={
          live_gates && (
            ...
            >
              ...
              {live_gates.eligible ? "Ir para Live" : `Live (${live_gates.checks_passed}/${live_gates.checks_total})`}
            </Link>
          )
        }
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 grid grid-cols-2 md:grid-cols-3 gap-4">
          <StatCard label="Lucro Acumulado" value={`${stats.pnl >= 0 ? "+" : ""}$${stats.pnl.toLocaleString()}`} delta={stats.pnl} icon={Wallet} accent="success" data={spark_up} />
          <StatCard label="Win Rate" value={`${stats.win_rate.toFixed(1)}%`} delta={stats.win_rate} icon={Target} accent="success" data={spark_up} />
          <StatCard label="Profit Factor" value={stats.profit_factor.toFixed(2)} delta={stats.profit_factor} icon={Gauge} accent="primary" data={spark_mix} />
          <StatCard label="Drawdown Atual" value={`${stats.drawdown.toFixed(1)}%`} delta={-stats.drawdown} icon={ShieldAlert} accent="warning" data={spark_down} />
          <StatCard label="Trades" value={String(stats.trades)} delta={stats.trades} icon={Activity} accent="secondary" data={spark_up} />
          <StatCard label="Dias em execução" value={String(stats.days_running)} delta={stats.days_running} icon={Calendar} accent="primary" data={spark_mix} />
        </div>
          ...
          <div className="flex flex-col items-center">
            <div className="relative h-48 w-48">
              <svg className="absolute inset-0 -rotate-90" viewBox="0 0 160 160">
                ...
                <motion.circle
                  cx="80" cy="80" r="70" fill="none" strokeWidth="10" strokeLinecap="round"
                  stroke="url(#scoreG)"
              ...
              <div className="absolute inset-0 grid place-items-center text-center">
                <div>
                  ...
                  <div className="text-[11px] uppercase tracking-wider text-muted-foreground">de 100</div>
                </div>
              </div>
            </div>
            <div className="mt-5 text-xs text-muted-foreground">{criteria_passed} de {criteria_total} critérios atendidos</div>
          </div>
        </Panel>
      </div>
          ...
          {criteria.map((c) => (
            ...
            >
              <div className="flex items-center gap-2.5">
                <span className={`h-2 w-2 rounded-full ${c.ok ? "bg-success" : "bg-warning"} animate-pulse`} />
