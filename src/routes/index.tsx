...
import {
  Wallet, TrendingUp, Brain, Target, Gauge, Activity,
  CheckCircle2, MonitorPlay, FlaskConical, Rocket,
} from "lucide-react";
import {
  Area, AreaChart, CartesianGrid, ResponsiveContainer, Tooltip, XAxis, YAxis,
  PolarAngleAxis, PolarGrid, Radar, RadarChart, PolarRadiusAxis,
} from "recharts";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Visão geral do desempenho do bot, estratégias ativas e operações." },
    ],
  }),
  component: Dashboard,
});
  ...
  if (isError && !data) {
    ...
    return (
      <div className="text-destructive text-sm space-y-2">
        <p>Erro ao carregar dashboard. Confirme que a API Python está ativa (<code className="text-secondary">python -m atlas.cli api</code>) e reinicie a UI (<code className="text-secondary">npm.cmd run dev</code>).</p>
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        actions={
          <div className="flex gap-2 flex-wrap">
            ...
            <button
              onClick={() => {
                ...
                bot.mutate(stats.bot_running ? "stop" : "start", {
                  onError: (e) => setBotErr(e instanceof Error ? e.message : "Falha ao controlar o bot"),
                });
              }}
            ...
            >
              {stats.kill_switch ? "Kill Switch" : stats.bot_running ? (isLive ? "Parar Live" : "Parar Bot") : "Iniciar Paper"}
            </button>
          </div>
        }
      />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-8 gap-4">
        <StatCard label="Saldo Total" value={`$${stats.balance.toLocaleString()}`} delta={stats.balance_delta_pct} icon={Wallet} accent="primary" data={spark_up} />
        <StatCard label="Lucro / Prejuízo" value={`${stats.pnl >= 0 ? "+" : ""}$${stats.pnl.toLocaleString()}`} delta={stats.pnl_delta_pct} icon={TrendingUp} accent="success" data={spark_up} />
        <StatCard label="Estratégia Ativa" value={stats.active_strategy.split(" ")[0]} icon={Brain} accent="secondary" data={spark_mix} />
        <StatCard label="Alignment Score" value={`${stats.alignment_score.toFixed(0)}`} icon={Gauge} accent="primary" data={spark_mix} />
        <StatCard label="Health Score" value={`${stats.health_score.toFixed(0)}`} icon={Target} accent="warning" data={spark_up} />
        <StatCard label="Win Rate" value={`${stats.win_rate_pct.toFixed(1)}%`} icon={CheckCircle2} accent="success" data={spark_mix} />
        <StatCard label="Profit Factor" value={stats.profit_factor.toFixed(2)} icon={Gauge} accent="warning" data={spark_up} />
              ...
              {RANGES.map((r) => (
                <button
                  ...
                  className={`text-xs px-3 py-1.5 rounded-lg transition ${
                    range === r ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-white"
                  }`}
                >
                  {r}
                </button>
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={drawdown_curve}>
                ...
                <Tooltip contentStyle={{ background: "#0f172a", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12 }} />
                <Area type="monotone" dataKey="drawdown_pct" stroke="#EF4444" fill="#EF4444" fillOpacity={0.2} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Panel>
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={quantum?.alignment_history ?? []}>
                ...
                <Tooltip contentStyle={{ background: "#0f172a", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12 }} />
                <Area type="monotone" dataKey="score" stroke="#7C3AED" fill="#7C3AED" fillOpacity={0.25} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          {quantum?.regime_label && (
            <p className="mt-3 text-xs text-muted-foreground">Regime atual: <span className="text-white">{quantum.regime_label}</span></p>
          )}
        </Panel>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            {flow.map((step) => (
              <div key={step.label} className="flex items-center gap-3">
                ...
                {step.pct >= 100 ? <CheckCircle2 className="h-4 w-4 text-success shrink-0" /> : null}
              </div>
            ))}
          </div>
          <div className="mt-4 flex gap-2 flex-wrap">
            <Link to="/backtests" className="text-xs text-secondary hover:underline inline-flex items-center gap-1"><FlaskConical className="h-3 w-3" /> Backtests</Link>
            <Link to="/validacao" className="text-xs text-secondary hover:underline inline-flex items-center gap-1"><MonitorPlay className="h-3 w-3" /> Validação</Link>
            <Link to="/live" className="text-xs text-secondary hover:underline inline-flex items-center gap-1"><Rocket className="h-3 w-3" /> Live</Link>
          </div>
        </Panel>
        ...
        <Panel title="Posições">
          {positions.length === 0 ? (
            ...
            positions.map((p, i) => (
              <div key={i} className="flex justify-between border-t border-white/5 first:border-t-0 py-3">
                <div>
                ...
                <div className={`text-right num ${p.pnl >= 0 ? "text-success" : "text-destructive"}`}>
